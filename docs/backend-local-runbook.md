...
后端在生产 profile 下会启动防呆检查：禁止 H2、禁止 H2 console、禁止默认 token secret、禁止默认 admin token、禁止 CORS 使用 `*`。如果配置不安全，应用会启动失败。
...
```powershell
$loginBody = @{ phone = '13800000000'; code = '123456' } | ConvertTo-Json
...
1. `src/data/services.ts` -> `GET /api/services`
...
- 当前手机号验证码未接短信平台，开发阶段接受任意非空验证码。
- 当前 token 是轻量 HMAC 令牌，生产建议升级为标准 JWT + refresh token 或接入统一认证。
- 当前支付是 mock 支付，生产必须接微信支付服务端验签和回调。
- 当前图片仍使用占位图，生产应接对象存储和 CDN。
- 当前没有管理后台，只保留了后续 Admin API 设计。
